    ...
    for (year,i) in zip(years,pages):
        ...
        for n in range(i):
           ...
           # if requests > 110:
            ...
            url = "https://www.moneycontrol.com/stocks/company_info/stock_news.php?sc_id=IT&scat=&pageno=" + str(page) + "&next=0&durationType=Y&Year=" + str(year) + "&duration=1&news_type="
            try:    
                driver.get(url)
            except Exception:
                print("ERROR")
...
df.to_csv('news_hero_19-16.csv', index=False, encoding='utf-8')    
print('---------DONE-----------')